...
function setDebugOptions(options/*opt*/)
{
   ...
   if ( ut.isStrValid(options) )
   {
      ...
      {
         var err = new TypeError("bad debug flags: `" + bad_flags + "'");
         err.bad_flags = bad_flags;
         throw err;
      }
...
//--------------------------------------------------------------------
//   Sends all further debug and trace output to a file, opened in append mode. 
...
//--------------------------------------------------------------------
// Write message @msg to the debug stream if it present. Otherwise function do nothing.
// The output formatting depends on debug options (see setDebugOptions description)
function debug(msg)
{
